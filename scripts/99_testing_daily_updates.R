...
df_23_04 <- fread("./data/treated/deaths_br_23_04_2020_by_gender_age_date_state.csv")
...
dates <- seq.Date(dmy("16/03/2020"), dmy("24/04/2020"), by = "day")
...
# We aim to compare data collected in 23/04 with the data collected
# in 24/04 to check if we need to append our daily data or if we 
# have to scrap all the data daily
df %>% 
  setNames(c("date", "state", "age_group", "gender", "number_deaths")) %>% 
  filter(date != dmy("24/04/2020")) %>% 
  mutate(day_collect = 24) %>% 
  bind_rows(df_23_04 %>% 
              as_tibble() %>% 
              mutate(date = ymd(date),
                     day_collect = 23)) %>% 
  group_by(day_collect, state) %>% 
  summarise(total_deaths = sum(number_deaths)) %>% 
  spread(day_collect, total_deaths) 

# we can see that our data is daily updated and we can have 
# updates in days gone by.
