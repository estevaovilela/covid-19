...
df_br_march_may <- df_br_march_may %>% 
  ungroup() %>% 
  mutate(capital = ifelse(capital == 1, "Capital of Metropolitan Area", "Metropolitan Area without Capital"))

ggplot() +
  geom_segment(data = df_br_march_may %>% select(rm, capital, dif) %>% spread(capital, dif) %>% arrange(rm),
               aes(x = rm, xend = rm, 
                   y = `Capital of Metropolitan Area`, yend = `Metropolitan Area without Capital`), 
               color = alpha("black", 0.7)) +
  geom_point(data = df_br_march_may %>% arrange(rm),
            aes(x = rm, y = dif, color = capital)) +
  coord_flip() +
  scale_y_continuous(breaks = seq(from = -40, to = 90, by = 10),
                     labels = as.character(seq(from = -40, to = 90, by = 10))) +
  labs(title = "Figure 1: Difference, in percent, between number of deaths registered from March to May of 2020 compared to 2019,\nCapital of Metropolitan Area and Metropolitan Area without Capital",
       subtitle = "Data collected on June, 6",
       x = "Metropolitan Area", 
...
df_br_january_may <- df_br_january_may %>% 
  ungroup() %>% 
  mutate(capital = ifelse(capital == 1, "Capital of Metropolitan Area", "Metropolitan Area without Capital"))

ggplot() +
  geom_segment(data = df_br_january_may %>% select(rm, capital, dif) %>% spread(capital, dif) %>% arrange(rm),
               aes(x = rm, xend = rm, 
                   y = `Capital of Metropolitan Area`, yend = `Metropolitan Area without Capital`), 
               color = alpha("black", 0.7)) +
  geom_point(data = df_br_january_may %>% arrange(rm),
            aes(x = rm, y = dif, color = capital)) +
  coord_flip() +
